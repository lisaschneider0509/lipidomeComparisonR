...
hclust_dendrogram <- function(hclust_element, 
  ...
  if(hclust_element$method == "complete"){
    ...
    plot_color <- mycolors[1]
  }
  else if(hclust_element$method == "single"){
    subtitle <- "Single linkage"
    plot_color <- mycolors[2]
  }
  else if(hclust_element$method == "average"){
    subtitle <- "Average linkage"
    plot_color <- mycolors[3]
  }
...
hclust_heatmap <- function(input_df, 
  ...
  out_file <- paste(out_path, "_hierarchical_heatmap", ".png", sep = "")
