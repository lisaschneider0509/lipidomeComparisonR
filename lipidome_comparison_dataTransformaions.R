...
#' Additionally the group variable is saved to each sample. Suitable for preprocessed csvs generated by compound discoverer. 
#' @param df data frame. Contains the columns c(Compound, Type, Filename, Status, Group, Area). 
#' @example #todo
flip_df <- function(df){
  sample <- levels(meat_target$Filename)
  new_df <- data.frame()
  for(i in 1:length(sample)){
    ...
    t_buffer_df <- add_column(t_buffer_df, SID = samples[i], .before = 1)
