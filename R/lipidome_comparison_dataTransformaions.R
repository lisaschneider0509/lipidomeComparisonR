...
#' get retention time
get_retention_time <- function(df, Compound, RT){
  sample <- levels(df$Compound)
  df$RT <- as.numeric(as.character(substr(lipid_list$RT, start = 1, stop = 4)))
  new_df <- data.frame()
  for(i in 1:length(sample)){
    buffer <- subset(df, subset = Compound == sample[i])
    new_df <- rbind(new_df, buffer[1, ])
  }
  new_df
}
