library(mixOmics)
X <- select_if(lipid_list, is.numeric)
Y <- lipid_list$Group
summary(Y)
dim(X); length(Y)
...
# df_out <- as.data.frame(MyResult.plsda$variates$X)
# df_out$group <- as.character(Y)
# colnames(df_out) <- c("PC1", "PC2", "group")
# p<-ggplot(df_out,
#           aes(x=PC1,y=PC2,color=group, label=row.names(df_out) )) + 
#   geom_point() + 
...
#           ellipse = TRUE, 
#           star = F, 
#           title = 'sPLS-DA on meat',
#           X.label = 'PLS-DA 1', 
#           Y.label = 'PLS-DA 2', 
#           col = viridis(n = 3, end = 1), 
#           style = "ggplot2", 
#           size.title = 10, 
#           pch.levels = c(15:17), 
#           point.lwd = 0.5, 
...
select.keepX
plot(tune.splsda.srbct,col = viridis(n = 3, alpha = 0.5))
